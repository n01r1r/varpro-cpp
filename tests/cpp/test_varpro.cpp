...
void close_matrix(const Matrix& actual, const Matrix& expected,
...
Problem exponential_problem(const Vector& t, const Matrix& observations,
                            bool offset = true) {
  Problem p;
...
Matrix exp_data(const Vector& t, const Vector& taus,
...
void check_exp_fit(const varpro::FitResult& result,
                   const Matrix& expected_coefficients,
  ...
  close_matrix(result.evaluation.coefficients, coefficients, 2e-6,
               label + " coefficients");
  close_matrix(result.evaluation.residuals,
               Matrix::Zero(result.evaluation.residuals.rows(),
...
void test_single_and_mrhs_fit() {
  ...
  truth << 1.0, 3.0;
  Matrix c(3, 3);
  c << 4.0, 2.0, -1.0, 2.5, 3.5, .75, 1.0, -0.5, 2.25;
...
void test_weighting_and_orthogonality() {
  ...
  close_matrix(aw.transpose() * rw, Matrix::Zero(3, 1), 2e-12,
               "weighted residual orthogonality");
  ...
  preweighted.basis = [base = weighted.basis, w](const Vector& x) {
    Matrix z = base(x);
    for (Eigen::Index i = 0; i < z.rows(); ++i) z.row(i) *= w(i);
    return z;
  };
  preweighted.derivative = [deriv = weighted.derivative, w](const Vector& x,
                                                            Eigen::Index k) {
    Matrix z = deriv(x, k);
    for (Eigen::Index i = 0; i < z.rows(); ++i) z.row(i) *= w(i);
    return z;
  ...
  for (Eigen::Index i = 0; i < y.rows(); ++i)
    preweighted.observations.row(i) *= w(i);
  auto ep = varpro::evaluate(preweighted, a);
  close_matrix(e.coefficients, ep.coefficients, 2e-12,
               "manual preweight coefficients");
  ...
  check(weighted_fit.converged() && preweighted_fit.converged(),
        "weighted fit convergence");
  close_matrix(weighted_fit.parameters, preweighted_fit.parameters, 2e-8,
               "manual preweight fitted parameters");
  close_matrix(weighted_fit.evaluation.coefficients,
               preweighted_fit.evaluation.coefficients, 2e-8,
               "manual preweight fitted coefficients");
  close_matrix(weighted_fit.evaluation.residuals,
               preweighted_fit.evaluation.residuals, 2e-8,
               "manual preweight fitted residuals");
}

void check_residual_jacobian(const Problem& problem, const Vector& parameters,
                             const LinearOptions& linear_options,
                             JacobianMode mode, double tolerance,
                             const std::string& label) {
  const auto evaluation =
      varpro::evaluate(problem, parameters, linear_options, mode);
  const double h = 1e-6;
  ...
  for (Eigen::Index k = 0; k < parameters.size(); ++k) {
    ...
    minus(k) -= h;
    Matrix difference =
        varpro::evaluate(problem, plus, linear_options, mode).residuals -
...
void test_kaufman_jacobians() {
  Vector t(9);
  for (Eigen::Index i = 0; i < t.size(); ++i) t(i) = i * .6;
  Vector a(2);
  a << 1.2, 3.7;
  Matrix c(3, 1);
  c << 2.3, -1.1, .4;
  ...
  Problem exact_mrhs =
      exponential_problem(t, exp_data(t, a, mrhs_coefficients));
  check_residual_jacobian(
      exact_mrhs, a, linear_options, JacobianMode::kaufman, 3e-7,
      "Kaufman zero-residual MRHS Jacobian finite difference");

  Problem off = exponential_problem(t, exp_data(t, a, c));
  off.observations(3, 0) += .13;
  off.observations(7, 0) -= .09;
  Vector x(2);
  x << 1.8, 5.5;
  ...
  for (Eigen::Index k = 0; k < 2; ++k) {
    ...
    minus(k) -= h;
    double fd_objective =
        (varpro::evaluate(off, plus, linear_options, JacobianMode::kaufman)
             .squared_error() -
         varpro::evaluate(off, minus, linear_options, JacobianMode::kaufman)
             .squared_error()) /
        (2.0 * h);
    double jacobian_objective =
        2.0 * eo.residuals.col(0).dot(eo.jacobian.col(k));
...
void test_exact_jacobians() {
  ...
  Problem exact = exponential_problem(t, exp_data(t, a, c));
  check_residual_jacobian(
      exact, a, linear_options, JacobianMode::retained_subspace, 3e-7,
      "retained-subspace zero-residual Jacobian finite difference");

  Matrix mrhs_coefficients(3, 4);
  mrhs_coefficients << 2.3, -.7, 1.1, .2, -1.1, 2.0, .3, -.4, .4, .6, -.2, 1.7;
  Problem exact_mrhs =
      exponential_problem(t, exp_data(t, a, mrhs_coefficients));
  check_residual_jacobian(
      exact_mrhs, a, linear_options, JacobianMode::retained_subspace, 3e-7,
  ...
  const auto exact_evaluation =
      varpro::evaluate(off, x, JacobianMode::retained_subspace);
  const auto compatibility_evaluation =
      varpro::evaluate(off, x, JacobianMode::exact);
  const auto kaufman_evaluation = varpro::evaluate(off, x);
  close_matrix(compatibility_evaluation.jacobian, exact_evaluation.jacobian,
               0.0, "exact compatibility alias");
  ...
  check_residual_jacobian(
      off, x, linear_options, JacobianMode::retained_subspace, 3e-7,
...
void test_rank_cases() {
  ...
  for (Eigen::Index i = 0; i < 6; ++i)
    deficient.observations(i, 0) =
        5.0 * std::exp(-t(i) / 2.0) + (i == 1 ? .2 : 0.0);
  deficient.basis = [t](const Vector& a) {
    Matrix b(t.size(), 2);
    for (Eigen::Index i = 0; i < t.size(); ++i)
      b.row(i) << std::exp(-t(i) / a(0)), std::exp(-t(i) / a(0));
    return b;
  };
  deficient.derivative = [t](const Vector& a, Eigen::Index k) {
    Matrix d = Matrix::Zero(t.size(), 2);
    if (k == 0)
      for (Eigen::Index i = 0; i < t.size(); ++i)
        d.row(i) << std::exp(-t(i) / a(0)) * t(i) / (a(0) * a(0)),
            std::exp(-t(i) / a(0)) * t(i) / (a(0) * a(0));
    return d;
  };
  auto e = varpro::evaluate(deficient, x);
  check(e.rank == 1, "deficient rank");
  check(e.diagnostics.singular_values.size() == 2,
        "deficient singular-value count");
  ...
  close(e.coefficients(0, 0), e.coefficients(1, 0), 2e-12,
        "minimum-norm duplicated coefficients");
  Matrix d = deficient.derivative(x, 0), phi = deficient.basis(x);
  const double expected_sum =
      phi.col(0).dot(deficient.observations.col(0)) / phi.col(0).squaredNorm();
  close(e.coefficients.col(0).sum(), expected_sum, 2e-10,
        "minimum-norm coefficient sum");
  close(e.coefficients(0, 0), expected_sum / 2.0, 2e-10,
        "minimum-norm coefficient value");
  Vector u = phi.col(0);
  u.normalize();
  ...
  zero.derivative = [](const Vector&, Eigen::Index) {
    return Matrix::Zero(6, 2);
  };
  auto z = varpro::evaluate(zero, x);
  check(z.rank == 0, "rank zero");
  check(z.diagnostics.singular_values.size() == 2,
        "rank-zero singular-value count");
  ...
  close(z.diagnostics.condition_estimate, 0.0, 0.0,
        "rank-zero condition estimate");
  close_matrix(z.coefficients, Matrix::Zero(2, 1), 0.0,
               "rank zero coefficients");
...
void test_scale_safe_rank_cutoff() {
  ...
  close(e.coefficients(0, 0) * 1e308, 1.0, 1e-12,
        "large-scale minimum-norm solve");
...
void test_configurable_rank_cutoff() {
  ...
  check(truncated.rank == 1,
        "configured rank cutoff truncates small singular value");
  close_matrix(truncated.coefficients, Matrix::Zero(2, 1), 0.0,
               "configured rank cutoff minimum-norm coefficients");
  Matrix expected_residuals(2, 1);
  expected_residuals << 0.0, 1.0;
...
void test_exact_jacobian_with_truncated_rotating_subspace() {
  ...
  // This is an intentional contract test: retained-subspace mode uses the
  // retained SVD formula and is not the derivative of the re-truncated residual
  // map when a nonzero singular direction is discarded.
  check((code.jacobian - finite_difference).cwiseAbs().maxCoeff() > 1e-4,
        "retained-subspace mode unexpectedly matches truncated residual finite "
        "difference");
...
void test_randomized_derivatives_and_invariants() {
  ...
  auto index = [&](int lower, int upper) {
    return lower + static_cast<int>(
                       rng() % static_cast<std::uint64_t>(upper - lower + 1));
  };

  for (int case_index = 0; case_index < 24; ++case_index) {
    const Eigen::Index n = index(1, 4);
    const Eigen::Index m =
        (std::max)(n + 3, static_cast<Eigen::Index>(index(6, 12)));
    ...
    for (Eigen::Index i = 0; i < m; ++i)
      for (Eigen::Index j = 0; j < n; ++j) base(i, j) = unit();
    base.topRows(n) += 2.0 * Matrix::Identity(n, n);
    for (Eigen::Index k = 0; k < q; ++k) {
      Matrix rate(m, n);
      for (Eigen::Index i = 0; i < m; ++i)
        for (Eigen::Index j = 0; j < n; ++j) rate(i, j) = 0.3 * unit();
    ...
    Problem problem;
    problem.basis = basis;
    problem.derivative = [basis, rates](const Vector& parameters,
                                        Eigen::Index k) {
    ...
    for (Eigen::Index i = 0; i < n; ++i)
      for (Eigen::Index j = 0; j < s; ++j) coefficients(i, j) = unit();
    ...
    check(kaufman.rank == n && exact.rank == n,
          "randomized case lost full numerical rank " +
    ...
    check(orthogonality <=
              1e-10 * (1.0 + weighted_basis.norm() * kaufman.residuals.norm()),
    ...
    for (Eigen::Index k = 0; k < q; ++k) {
      ...
      check(basis_error <= 1e-8, "randomized basis derivative " +
                                     std::to_string(case_index) + "/" +
                                     std::to_string(k));
      ...
      const double finite_difference_error =
          (plus_evaluation.squared_error() - minus_evaluation.squared_error()) /
          (2.0 * h);
...
template <class F>
void expect_invalid(F&& f, const std::string& name) {
  try {
    f();
...
template <class F>
void expect_domain(F&& f, const std::string& name) {
  try {
    f();
...
void test_errors_and_limit() {
  Vector t(5);
  for (Eigen::Index i = 0; i < 5; ++i) t(i) = static_cast<double>(i);
  Vector truth(2);
  truth << 1.0, 3.0;
  Matrix truth_coefficients(3, 1);
  truth_coefficients << 4.0, 2.5, 1.0;
  ...
  Problem missing_derivative = p;
  missing_derivative.derivative = {};
  expect_invalid([&] { varpro::evaluate(missing_derivative, a); },
                 "missing derivative callback");
  Problem bad_data = p;
  bad_data.observations.resize(0, 1);
  expect_invalid([&] { varpro::evaluate(bad_data, a); }, "empty observations");
  bad_data = p;
  bad_data.observations(0, 0) = std::numeric_limits<double>::quiet_NaN();
  expect_invalid([&] { varpro::evaluate(bad_data, a); },
                 "nonfinite observations");
  Problem bad_shape = p;
  bad_shape.basis = [](const Vector&) { return Matrix::Ones(4, 3); };
  expect_invalid([&] { varpro::evaluate(bad_shape, a); }, "basis shape");
  Problem bad_weight = p;
  bad_weight.weights = Vector::Zero(5);
  expect_invalid([&] { varpro::evaluate(bad_weight, a); }, "all-zero weights");
  bad_weight.weights = Vector::Constant(5, -1.);
  expect_invalid([&] { varpro::evaluate(bad_weight, a); }, "negative weights");
  bad_weight.weights =
      Vector::Constant(5, std::numeric_limits<double>::quiet_NaN());
  ...
  Problem nan_basis = p;
  nan_basis.basis = [](const Vector&) {
    return Matrix::Constant(5, 3, std::numeric_limits<double>::infinity());
  };
  expect_domain([&] { varpro::evaluate(nan_basis, a); }, "nonfinite basis");
  Problem nan_derivative = p;
  nan_derivative.derivative = [](const Vector&, Eigen::Index) {
    return Matrix::Constant(5, 3, std::numeric_limits<double>::quiet_NaN());
  };
  expect_domain([&] { varpro::evaluate(nan_derivative, a); },
                "nonfinite derivative");
  varpro::Options invalid_options;
  invalid_options.max_evaluations = 0;
  expect_invalid([&] { varpro::fit(p, a, invalid_options); },
                 "invalid options");
  invalid_options = {};
  invalid_options.ftol = -1.0;
  expect_invalid([&] { varpro::fit(p, a, invalid_options); },
                 "negative tolerance");
  invalid_options = {};
  invalid_options.linear_options.rcond =
      std::numeric_limits<double>::quiet_NaN();
  expect_invalid([&] { varpro::fit(p, a, invalid_options); },
                 "nonfinite rank cutoff");
  invalid_options = {};
  invalid_options.jacobian_mode = static_cast<JacobianMode>(99);
  expect_invalid([&] { varpro::fit(p, a, invalid_options); },
                 "invalid Jacobian mode");

  varpro::Options limited;
  limited.max_evaluations = 1;
  auto r = varpro::fit(p, a, limited);
  check(r.status == varpro::Status::evaluation_limit,
        "evaluation-limit status");
  ...
  close_matrix(r.evaluation.coefficients, final_eval.coefficients, 1e-12,
               "final coefficients consistency");
  close_matrix(r.evaluation.residuals, final_eval.residuals, 1e-12,
...
int main() {
  const std::vector<std::pair<const char*, std::function<void()>>> tests = {
      ...
      {"randomized derivatives and invariants",
       test_randomized_derivatives_and_invariants},
