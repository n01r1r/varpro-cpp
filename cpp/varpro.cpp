...
bool finite(const Matrix& matrix) { return matrix.array().isFinite().all(); }

bool finite(const Vector& vector) { return vector.array().isFinite().all(); }
...
void validate_inputs(const Problem& problem, const Vector& parameters) {
  ...
  if (problem.weights.size() != 0 && problem.weights.size() != m)
    throw std::invalid_argument(
        "weights must be empty or have one entry per observation");
...
Index stacked_size(Index rows, Index columns) {
  ...
  if (rows <= 0 || columns <= 0 || rows > maximum / columns)
    throw std::invalid_argument(
        "stacked residual dimensions overflow Eigen::Index");
  return rows * columns;
}
...
Matrix apply_weights(const Matrix& matrix, const Vector& weights) {
  ...
  if (weights.size() == 0) return weighted;
  for (Index i = 0; i < weighted.rows(); ++i) weighted.row(i) *= weights(i);
  return weighted;
}
...
Prepared prepare(const Problem& problem, const Vector& parameters,
  ...
  if (weighted_observations->rows() != m ||
      weighted_observations->cols() != s || !finite(*weighted_observations))
    throw std::domain_error(
        "weighted observations are nonfinite or have invalid shape");
  ...
  // pseudoinverse.
  Eigen::JacobiSVD<Matrix> svd(weighted_basis,
  ...
  const double relative_cutoff =
      linear_options.rcond < 0.0 ? static_cast<double>((std::max)(m, n)) *
  ...
  if (rank != 0) {
    ...
    Matrix projected_rhs =
        result.retained_u.transpose() * *weighted_observations;
  ...
  result.residuals =
      *weighted_observations - weighted_basis * result.coefficients;
...
Matrix form_jacobian(const Problem& problem, const Vector& parameters,
                     Prepared& prepared, JacobianMode jacobian_mode) {
  ...
  for (Index k = 0; k < q; ++k) {
    ...
    if (derivative.rows() != m || derivative.cols() != prepared.n)
      throw std::invalid_argument(
          "derivative must have the same m x n shape as basis");
    const Matrix weighted_derivative =
        apply_weights(derivative, problem.weights);
    ...
    if (s <= prepared.n) {
      const Matrix derivative_coefficients =
          weighted_derivative * prepared.coefficients;
      if (!finite(derivative_coefficients))
        throw std::domain_error("Jacobian intermediate is nonfinite");
      column_matrix = prepared.retained_u * (prepared.retained_u.transpose() *
                                             derivative_coefficients) -
                      derivative_coefficients;
    } else {
      const Matrix projected_derivative =
          prepared.retained_u *
              (prepared.retained_u.transpose() * weighted_derivative) -
          weighted_derivative;
      if (!finite(projected_derivative))
        throw std::domain_error("Jacobian intermediate is nonfinite");
      column_matrix = projected_derivative * prepared.coefficients;
    }
    if (jacobian_mode == JacobianMode::retained_subspace &&
        prepared.rank != 0) {
      ...
      if (!finite(correction))
        throw std::domain_error(
            "retained-subspace Jacobian intermediate is nonfinite");
...
class FitFunctor final : public Eigen::DenseFunctor<double> {
 public:
  FitFunctor(const Problem& problem, const Options& options, int inputs,
             int values, Matrix weighted_observations)
      : Eigen::DenseFunctor<double>(inputs, values),
        problem_(problem),
        options_(options),
        parameter_count_(inputs),
        weighted_observations_(std::move(weighted_observations)) {}
 ...
 private:
  Prepared& prepared_with_jacobian(const Vector& parameters) {
    ...
    if (!current.has_jacobian) {
      ...
      current.jacobian =
          form_jacobian(problem_, parameters, current, options_.jacobian_mode);
  ...
  Prepared& prepared_for(const Vector& parameters) {
    ...
    if (cached_ && same_vector(cached_parameters_, parameters)) return *cached_;

    Prepared fresh = prepare(problem_, parameters, options_.linear_options,
                             expected_n_, &weighted_observations_);
    ...
    if (expected_n_ == 0) expected_n_ = fresh.n;
...
Status map_status(Eigen::LevenbergMarquardtSpace::Status status,
...
FitResult fit(const Problem& problem, Vector initial_parameters,
  ...
  const Index stacked =
      stacked_size(problem.observations.rows(), problem.observations.cols());
  const Index int_max = (std::numeric_limits<int>::max)();
  if (q > int_max || stacked > int_max || stacked < q)
    throw std::invalid_argument(
        "fit dimensions are incompatible with Eigen LM");
  ...
  FitFunctor functor(problem, options, static_cast<int>(q),
                     static_cast<int>(stacked),
  ...
  const Eigen::LevenbergMarquardtSpace::Status lm_status =
